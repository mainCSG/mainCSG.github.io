...
class PotentialInterpolator:
    ...
    # indirectly defining call method for the class so that it can be overridden
    def __call__(self, volt_vec_input):
        try:    
            return self.call_method(volt_vec_input)     # in case it's overridden
        except:
            return self.call_method(self, volt_vec_input)

    @staticmethod
    def call_method(self, volt_vec_input):
        '''
    ...
    def plot(self, volt_vec, plot_type='2D', y_slice=0, x_slice=None,
        ...
        # Do a 2D plot
            ...
            if not show_wf:
                ...
                fig.colorbar(img, ax=ax,location='right', shrink=0.6)
                ax.set(xlabel='x-coords [nm]',ylabel='y-coords [nm]',
                       title='2D potential profile [ J ]')
            # Overlay wavefunction if desired
            else:
                ...
                fig.colorbar(im0, ax=ax[0],location='right', shrink=0.6)
                ...
                fig.colorbar(im1, ax=ax[1], location='right', shrink=0.6)
